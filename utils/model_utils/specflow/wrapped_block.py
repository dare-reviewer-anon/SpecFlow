...
class DAREWrappedBlock(nn.Module):
    ...
    def __init__(self, base_block: nn.Module, specflow_controller: DAREController, layer_idx: int):
        super().__init__()
        self.block = base_block
        self.specflow = specflow_controller
    ...
    def forward(
        ...
        exec_mask = specflow_out["exec_mask"]  # [B, T]
        hidden_states = hidden_states * exec_mask.unsqueeze(-1)

        aux_losses: Dict[str, torch.Tensor] = {
            "L_text_ratio": specflow_out["L_text_ratio"],
            "L_vis_soft": specflow_out["L_vis_soft"],
            "L_vis_hard": specflow_out["L_vis_hard"],
        }
